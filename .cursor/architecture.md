...
| **2 — Checklist soft Accueil** | Compléments non bloquants visibles | Bandeau / cartes soft : téléphone, contact urgence, voix de rappel, photo (si absents) — dismissible ; liens vers écrans Profil existants | Patient voit ≤ 4 nudges utiles sans bloquer les doses |
| **3 — Gestion traitements & médicaments** | Ne plus être create-only | Éditer / supprimer médicament + horaires (API PATCH/DELETE) ; suspendre / modifier phase & `date_fin_prevue` ; garder « terminé » | Un patient peut corriger un horaire sans tout recréer ; suspendre un traitement marche |
| **4 — Profil & compte** | Compte utilisable au quotidien | Photo profil (`photo_url`) ; changement email / mot de passe (contrat auth d’abord si manquant) ; verrou local PIN ou biométrie (skill auth §5) ; CGU ouvrable | Profil n’a plus de tuiles mortes critiques |
| **5 — Sync visible & polish** | Confiance hors-ligne + finitions | Bandeau offline / outbox en attente ; historique check-in ; surface stock bas (Accueil ou Santé) ; corriger `onViewAll` / Santé = constantes (raccourcis soins optionnels) | Utilisateur comprend s’il est offline ; dettes UI listées corrigées |
